 ...
 *
 *   const ctx = new ScenarioContext('C4-001', 'TC-INFRA-MCI-03');
 *
 *   test.beforeAll(async () => {
 *     const mciId = ctx.store.wasBypassed('TC-INFRA-MCI-02')
 *       ? ctx.store.getOrDefault('nsId', 'default')
 *       : ctx.store.require('nsId');
 *   });
 *
 *   test.afterAll(async () => {
 ...
 *
 *   const ctx = new StandaloneContext('TC-INFRA-MCI-03', 'aws');
