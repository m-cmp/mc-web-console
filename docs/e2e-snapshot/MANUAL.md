...
| `ready` | 구현 완료, 정상 실행 | ✓ | ✓ |
| `wip` | 구현 중 | ✓ (부분) | △ 일부 |
| `bypass` | 미구현·미지원 — 시나리오에서 skip | 있어도 빈 stub | ✗ |
| `todo` | 구현 예정 | ✗ | ✗ |
...
|---|---|
| `ready` | 전체 스텝 실행 가능 |
| `partial` | 일부 스텝 bypass 포함, 나머지 실행 가능 |
| `wip` | 작업 중 |
| `todo` | 구현 예정 |
| `deprecated` | 더 이상 사용하지 않음 |

> **규칙**: 스텝 중 하나라도 `bypass`가 있으면 시나리오 status는 `partial` 이상이어야 한다.
...
### 4-3. bypass된 TC의 값을 안전하게 읽기

앞 TC가 bypass된 경우 OUT param이 없을 수 있다. `wasBypassed` + `getOrDefault`를 사용한다.

```typescript
// TC-INFRA-MCI-03 이 bypass 되었을 수도 있는 경우
test.beforeAll(() => {
  if (!ctx) { mciId = process.env.MCI_ID ?? 'fallback'; return; }

  mciId = ctx.store.wasBypassed('TC-INFRA-MCI-03')
    ? ctx.store.getOrDefault<string>('mciId', 'fallback-mci')
    : ctx.store.require<string>('mciId');
});
```

bypass 처리한 TC에서는 반드시 `setBypassed`를 호출한다:

```typescript
// deploy/tc/infra/TC-INFRA-MCI-03.spec.ts (bypass stub)
test.skip(true, 'TC-INFRA-MCI-03: bypass — infra pre-provisioned');

test.afterAll(() => {
  ctx?.store.setBypassed('TC-INFRA-MCI-03');
});
```
...
**Q. bypass 상태인 TC는 어떻게 처리하나?**

- spec 파일에 빈 stub만 두고 `test.skip(true, 'bypass 이유')` 처리
- 레지스트리에 `bypass: { reason: '...', issue: 'ISSUE-XXX' }` 필드 기재
- 시나리오에서는 해당 스텝의 status를 `'bypass'`로 설정
- 런타임 store에서 `ctx.store.setBypassed('TC-ID')` 를 호출하면 후속 TC가 `getOrDefault`로 안전하게 읽음
...
| `bypass` | 등록 완료 (bypass 사유 명시) | stub | skip 후 다음 스텝 진행 |
| `todo` | 등록 완료 | 없음 | skip |
