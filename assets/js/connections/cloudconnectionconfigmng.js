...
$(document).ready(function () {
    ...
    $('#alertResultArea').addEventListener('hidden.bs.modal', function () {// bootstrap 3 또는 4
        ...
        let targetUrl = "/settings/connection/mngform"
    ...
    $(window).on("load resize", function () {
      ...
      // mcpjs["util/common"].setTableHeightForScroll('connectionListTable', 300)
    });
...
function getCloudConnectionList(sort_type) {
    ...
    //                     +'<td class="btn_mtd ovm td_left" data-th="Name">'+item.ConfigName+'<a href="javascript:void(0);"><img src="/assets/images/contents/icon_copy.png" class="td_icon" alt=""/></a> <span class="ov"></span></td>'
    ...
    //                     +'<td class="overlay hidden" data-th=""><a href="javascript:void(0);"><img src="/assets/images/contents/icon_link.png" class="icon" alt=""/></a></td>' 
    //                     +'</tr>' 
    //             ))
    //         }else{
...
function addCloudConnectionConfirm() {
    ...
    //ok 위치이동
    // $('#RegistBox').addEventListener('hidden.bs.modal', function () {
    console.log(" ok 눌렀음");// cancel 눌러도 들어옴.
    var offset = $("#CreateBox").offset();
    $("#wrap").animate({ scrollTop: offset.top }, 300);
    // })

  // mcpjs["util/common"].setTableHeightForScroll("regionListTable", 300);
  // mcpjs["util/common"].setTableHeightForScroll("credentialListTable", 300);
  // mcpjs["util/common"].setTableHeightForScroll("driverListTable", 300);
}
...
function createCloudConnection() {
    ...
    if (!configname) {
      mcpjs["util/util"].commonAlert("Input New Cloud Connection Name")
        $("#regConfigName").focus()
        return;
    }
    if (!providername) {
      mcpjs["util/util"].commonAlert("Input Provider Name")
        $("#regProvider").focus()
        return;
    }

    if (!drivername) {
      mcpjs["util/util"].commonAlert("Select Driver")
        return;
    }

    if (!credentialname) {
      mcpjs["util/util"].commonAlert("Select Credential")
        return;
    }

    if (!regionname) {
      mcpjs["util/util"].commonAlert("Select Resion/Zone")
        return;
    }


    //var url = "/settings/connection/cloudconnectionconfig" + "/reg/proc";
    var obj = {
    ...
    if (configname) {
        var controllerKeyName = "CloudConnectionCreate";
	    var optionParamMap = new Map();
        mcpjs["util/pathfinder"].postCommonData('saveConnection',controllerKeyName,optionParamMap,obj, mcpjs['connections/cloudconnectionconfigmng'].menuRegCallbackSuccess)
      ...
      mcpjs["util/util"].commonAlert("Input Cloud Connection Name")
        $("#regConfigName").focus()
        return;
    }
}
...
// 메뉴
export function menuRegCallbackSuccess(caller, result){
    if (result.status == 200 || result.status == 201) {
        mcpjs["util/util"].commonResultAlert("Success Create Cloud Connection")
        //등록하고 나서 화면을 그냥 고칠 것인가?
        //getCloudConnectionList();
        //아니면 화면을 리로딩 시킬것인가?
        //location.reload();
        ...
        mcpjs["util/util"].commonResultAlert("Fail Create Cloud Connection")
    }
}
...
function deleteCloudConnection() {
    ...
    if (cnt < 1) {
      mcpjs["util/util"].commonAlert("삭제할 대상을 선택해 주세요.");
        return;
    }
    if (cnt > 1) {
      mcpjs["util/util"].commonAlert("한개씩만 삭제 가능합니다.")
        return;
    }
    ...
    if (cnt == 1) {
        ...
        var url = "/settings/connection/cloudconnectionconfig" + "/del/" + mcc_id;
        // if(confirm("삭제하시겠습니까?")){
        client.delete(url, {
            headers: {
                'Content-type': 'application/json',
            ...
            if (result.status == 200) {
                ...
                mcpjs["util/util"].commonResultAlert("Deleted Cloud Connection ")
...
function getRegionDetail(target) {
    var url = "/settings/connection/region/" + target;
...
function saveNewRegion() {
    ...
    if (providerName == "AZURE") {
        if (!providerName || !locationID || !resourceGroupID) {
          ...
          mcpjs["util/util"].commonAlert("Please enter the required items.")
            return;
        }
        keyName1 = "location"
    ...
    var controllerKeyName = "RegionCreate";
    var optionParamMap = new Map();
    mcpjs["util/pathfinder"].postCommonData('saveConnection',controllerKeyName,optionParamMap, regionInfo, mcijs['connections/cloudconnectionconfigmng'].regionRegCallbackSuccess)

    // client.post("/settings/connection/region/reg/proc", regionInfo, {

    // }).then(result => {
    //     console.log(result);
    //     if (result.status == 200 || result.status == 201) {
    //       mcpjs["util/util"].commonAlert("Success Save Cloud Region");
    //         // 성공하면 내용 초기화
    //         $("#RegionModalRegionName").val('');
    //         $("#RegionModalProviderName option:eq(0)").attr("select", "selected");
    //         $("#RegionModalRegionID").val('');
    //         $("#RegionModalZoneID").val('');
    //         // Region table 갱신
    //         getRegionList();
    //     } else {
    //       mcpjs["util/util"].commonAlert("Fail Create Cloud Region")
    //     }
...
// region 등록 성공 시
export function regionRegCallbackSuccess(caller, result){
    if (result.status == 200 || result.status == 201) {
        mcpjs["util/util"].commonAlert("Success Save Cloud Region");
          // 성공하면 내용 초기화
          $("#RegionModalRegionName").val('');
...
function deleteRegion() {
    ...
    if (!regionName) {
      ...
      mcpjs["util/util"].commonAlert("No Region selected")
        return;
    }
    // $("#requireMessage").text("")
    ...
    //RegionDelete
    var url = "/settings/connection/region/del/" + regionName;
    //client.post("/settings/connection/region/reg/proc",regionInfo,{
    client.delete(url, {}, {
    }).then(result => {
        console.log(result);
        if (result.status == 200 || result.status == 201) {
          mcpjs["util/util"].commonAlert("Deleted Cloud Region");
            // Region 갱신 
            getRegionList();
        } else {
          mcpjs["util/util"].commonAlert("Fail to delete the Cloud Region")
        }
        // }).catch(function(error){
...
function saveNewCredential() {
    ...
    var controllerKeyName = "CredentialCreate";
    var optionParamMap = new Map();
    mcpjs["util/pathfinder"].postCommonData('saveCredential',controllerKeyName,optionParamMap,credentialInfo, mcpjs['connections/cloudconnectionconfigmng'].credentialRegCallbackSuccess)

    // client.post("/settings/connection/credential/reg/proc", credentialInfo, {
    //     // headers: { 'content-type': 'application/x-www-form-urlencoded' },
    // }).then(result => {
    //     console.log(result);
    //     if (result.status == 200 || result.status == 201) {
    //       mcpjs["util/util"].commonAlert("Success Save Cloud Credential");
    ...
    //         getCredentialList();
    //     } else {
    //       mcpjs["util/util"].commonAlert("Fail Create Cloud Credential")
    //     }
...
// credential 등록성공 
export function credentialRegCallbackSuccess(caller, result){
    console.log(result);
    if (result.status == 200 || result.status == 201) {
        mcpjs["util/util"].commonAlert("Success Save Cloud Credential");
        // // 성공하면 내용 초기화 : provider가 같으면 key0, key1 은 그대로 사용
...
function deleteCredential() {
    ...
    if (!credentialName) {
        ...
        // $("#requiredCloudConnection").modal()
      mcpjs["util/util"].commonAlert("No Credential selected")
        return;
    }
    // $("#requireMessage").text("")
    ...
    var url = "/settings/connection/credential/del/" + credentialName;
    client.delete(url, {}, {
    }).then(result => {
        console.log(result);
        if (result.status == 200 || result.status == 201) {
          mcpjs["util/util"].commonAlert("Deleted Cloud Credential");
            // Credential 갱신 
            getCredentialList();
        } else {
          mcpjs["util/util"].commonAlert("Fail to delete the Cloud Credential")
        }
        // }).catch(function(error){
        //     console.log("delete error : ",error);        
        // });
...
function saveNewDriver() {
    ...
    //DriverCreate
    var controllerKeyName = "DriverCreate";
    var optionParamMap = new Map();
    mcpjs["util/pathfinder"].postCommonData('saveDriver',controllerKeyName,optionParamMap,driverlNameInfo, mcpjs['connections/cloudconnectionconfigmng'].driverRegCallbackSuccess)

    // client.post("/settings/connection/driver/reg/proc", driverlNameInfo, {

    // }).then(result => {
    //     console.log(result);
    //     if (result.status == 200 || result.status == 201) {
    //       mcpjs["util/util"].commonAlert("Success Save Cloud Driver");
    //         // 성공하면 내용 초기화
    ...
    //         getDriverList();
    //     } else {
    //       mcpjs["util/util"].commonAlert("Fail Create Cloud Driver")
    //     }
...
// Driver 등록 성공
export function driverRegCallbackSuccess(caller, result){
    console.log(result);
    if (result.status == 200 || result.status == 201) {
        mcpjs["util/util"].commonAlert("Success Save Cloud Driver");
        // 성공하면 내용 초기화
...
function deleteDriver() {
    ...
    if (!driverName) {
        ...
        // $("#requiredCloudConnection").modal()
      mcpjs["util/util"].commonAlert("No driver selected")
        return;
    }
    // $("#requireMessage").text("")
    ...
    var url = "/settings/connection/driver/del/" + driverName;
    client.delete(url, {}, {
    }).then(result => {
        console.log(result);
        if (result.status == 200 || result.status == 201) {
          mcpjs["util/util"].commonAlert("Deleted Cloud Driver");
            // Driver 갱신 
            getDriverList();
        } else {
          mcpjs["util/util"].commonAlert("Fail to delete the Cloud Driver")
        }
        // }).catch(function(error){
        //     console.log("delete error : ",error);        
        // });
...
function setInspectResourcesList() {
    ...
    var url = "/settings/resources/inspectresources/list"
    ...
    client.post(url, obj, {
        ...
        if (result.status == 200 || result.status == 201) {
          ...
          mcpjs["util/util"].commonAlert("Set Inspect Resource List Fail")
        }
...
function setInspectResourcesListAll() {
    var url = "/settings/resources/inspectresourcesoverview"
