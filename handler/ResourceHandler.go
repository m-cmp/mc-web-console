...
func RegFirewallRules(nameSpaceID string, securityGroupID string, firewallRuleReq *tbmcir.TbFirewallRulesWrapper) (*tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &securityGroupInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	if respStatus != 200 && respStatus != 201 {
		...
		return &securityGroupInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: errorInfo.Message}
	}
	...
	return &securityGroupInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func DelFirewallRules(nameSpaceID string, securityGroupID string, firewallRuleReq *tbmcir.TbFirewallRulesWrapper) (*tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/securityGroup/{securityGroupId}/rules"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	paramMapper["{securityGroupId}"] = securityGroupID
	...
	if respStatus != 200 && respStatus != 201 {
		return &securityGroupInfo, frameworkmodel.WebStatus{StatusCode: respStatus, Message: resultInfo.Message}
	}
	json.NewDecoder(respBody).Decode(&securityGroupInfo)
	fmt.Println(securityGroupInfo)
	return &securityGroupInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
...
func GetVnetList(nameSpaceID string) ([]tbmcir.TbVNetInfo, frameworkmodel.WebStatus) {
	...
	return vNetInfoList["vNet"], frameworkmodel.WebStatus{StatusCode: respStatus}
}

// ID목록만 조회
func GetVnetListByID(nameSpaceID string, filterKeyParam string, filterValParam string) ([]string, frameworkmodel.WebStatus) {
	fmt.Println("GetVnetList ************ : ")
	var originalUrl = "/ns/{nsId}/resources/vNet"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	...
	//return vNetInfoList["idList"], frameworkmodel.WebStatus{StatusCode: respStatus}
	return vNetInfoList.IDList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// List 조회시 optionParam 추가
func GetVnetListByOption(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]tbmcir.TbVNetInfo, frameworkmodel.WebStatus) {
...
func GetVpcData(nameSpaceID string, vNetID string) (*tbmcir.TbVNetInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &vNetInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	// return vNetInfo, err
	return &vNetInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// vpc 등록
// option=register 항목은 TB에서 자동으로 넣을 때 사용하는 param으로 webtool에서 사용하지 않음.
func RegVpc(nameSpaceID string, vnetRegInfo *tbmcir.TbVNetReq) (*tbmcir.TbVNetInfo, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/vNet"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	urlParam := util.MappingUrlParameter(originalUrl, paramMapper)
...
// vpc 삭제
func DelVpc(nameSpaceID string, vNetID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	// if ValidateString(vNetID) != nil {
	if len(vNetID) == 0 {
		log.Println("vNetID 가 없으면 해당 namespace의 모든 vpc가 삭제되므로 처리할 수 없습니다.")
		return webStatus, frameworkmodel.WebStatus{StatusCode: 4040, Message: "vNetID 가 없으면 해당 namespace의 모든 vpc가 삭제되므로 처리할 수 없습니다."}
	}
	var originalUrl = "/ns/{nsId}/resources/vNet/{vNetId}"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
...
// 전체 vpc 삭제
func DelAllVpc(nameSpaceID string) (tbcommon.TbSimpleMsg, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/vNet"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
...
func GetSecurityGroupList(nameSpaceID string) ([]tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
	...
	return securityGroupList["securityGroup"], frameworkmodel.WebStatus{StatusCode: respStatus}
}

// ID만 조회
func GetSecurityGroupListByOptionID(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]string, frameworkmodel.WebStatus) {
	...
	//return securityGroupList["idList"], frameworkmodel.WebStatus{StatusCode: respStatus}
	return securityGroupList.IDList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// SecurityGroupList 조회 시 Option에 해당하는 값만 조회. GetSecurityGroupList와 TB 호출은 동일하나 option 사용으로 받아오는 param이 다름
func GetSecurityGroupListByOption(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
	...
	if filterKeyParam != "" {
		if filterKeyParam == "vpcId" {
			filterKeyParam = "vNetId" // tb는 vNetId로 지칭하고 mcon에서는 vpc로 지칭함
		}
		urlParam += "&filterKey=" + filterKeyParam
		urlParam += "&filterVal=" + filterValParam
	}
...
// SecurityGroup 상세 조회
func GetSecurityGroupData(nameSpaceID string, securityGroupID string) (*tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
...
// SecurityGroup 등록
// option=register 항목은 TB에서 자동으로 넣을 때 사용하는 param으로 webtool에서 사용하지 않음.
func RegSecurityGroup(nameSpaceID string, securityGroupRegInfo *tbmcir.TbSecurityGroupReq) (*tbmcir.TbSecurityGroupInfo, frameworkmodel.WebStatus) {
...
func DelAllSecurityGroup(nameSpaceID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/securityGroup"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	urlParam := util.MappingUrlParameter(originalUrl, paramMapper)
...
// SecurityGroup 삭제
func DelSecurityGroup(nameSpaceID string, securityGroupID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	// if ValidateString(vNetID) != nil {
	if len(securityGroupID) == 0 {
		log.Println("securityGroupID 가 없으면 해당 namespace의 모든 securityGroup이 삭제되므로 처리할 수 없습니다.")
		return webStatus, frameworkmodel.WebStatus{StatusCode: 4040, Message: "securityGroupID 가 없으면 해당 namespace의 모든 securityGroup이 삭제되므로 처리할 수 없습니다."}
	}
...
func GetSshKeyInfoList(nameSpaceID string) ([]tbmcir.TbSshKeyInfo, frameworkmodel.WebStatus) {
	...
	return sshKeyList["sshKey"], frameworkmodel.WebStatus{StatusCode: respStatus}

}

func GetSshKeyInfoListByID(nameSpaceID string, filterKeyParam string, filterValParam string) ([]string, frameworkmodel.WebStatus) {
	log.Println("GetSshKeyInfoListByID ************ : ")
	...
	//return sshKeyList["idList"], frameworkmodel.WebStatus{StatusCode: respStatus}
	return sshKeyList.IDList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func GetSshKeyInfoListByOption(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]tbmcir.TbSshKeyInfo, frameworkmodel.WebStatus) {
	fmt.Println("GetSshKeyInfoList ************ : ")
	var originalUrl = "/ns/{nsId}/resources/sshKey"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
...
// sshKey 상세 조회
func GetSshKeyData(nameSpaceID string, sshKeyID string) (*tbmcir.TbSshKeyInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &sshKeyInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	return &sshKeyInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// sshKey 등록
// option=register 항목은 TB에서 자동으로 넣을 때 사용하는 param으로 webtool에서 사용하지 않음.
func RegSshKey(nameSpaceID string, sshKeyRegInfo *tbmcir.TbSshKeyReq) (*tbmcir.TbSshKeyInfo, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/sshKey"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	urlParam := util.MappingUrlParameter(originalUrl, paramMapper)
...
func UpdateSshKey(nameSpaceID string, sshKeyId string, sshKeyInfo *tbmcir.TbSshKeyInfo) (*tbmcir.TbSshKeyInfo, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/sshKey/{sshKeyId}"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	paramMapper["{sshKeyId}"] = sshKeyId
	...
	if err != nil {
		fmt.Println(err)
		return &sshKeyInfoResponse, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	log.Println("resp = ", resp)
	respBody := resp.Body
	respStatus := resp.StatusCode
	...
	return &sshKeyInfoResponse, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// sshKey 삭제
func DelSshKey(nameSpaceID string, sshKeyID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	// if ValidateString(sshKeyID) != nil {
	if len(sshKeyID) == 0 {
		log.Println("sshKeyID 가 없으면 해당 namespace의 모든 sshKeyID 삭제되므로 처리할 수 없습니다.")
		return webStatus, frameworkmodel.WebStatus{StatusCode: 4040, Message: "sshKeyID 가 없으면 해당 namespace의 모든 sshKeyID 삭제되므로 처리할 수 없습니다."}
	}
...
// 전체 sshKey 삭제
func DelAllSshKey(nameSpaceID string) (tbcommon.TbSimpleMsg, frameworkmodel.WebStatus) {
	var originalUrl = "/ns/{nsId}/resources/sshKey"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
...
func GetVirtualMachineImageInfoList(nameSpaceID string) ([]tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
	...
	return virtualMachineImageList["image"], frameworkmodel.WebStatus{StatusCode: respStatus}
}

// VirtualMachineImage 목록에서 Option으로 ID 목록만 가져오는 function
func GetVirtualMachineImageInfoListByID(nameSpaceID string, filterKeyParam string, filterValParam string) ([]string, frameworkmodel.WebStatus) {
	...
	return virtualMachineImageIdList.IDList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func GetVirtualMachineImageInfoListByOption(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
...
func GetVirtualMachineImageData(nameSpaceID string, virtualMachineImageID string) (*tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &virtualMachineImageInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	return &virtualMachineImageInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func UpdateVirtualMachineImage(nameSpaceID string, virtualMachineImageID string, imageInfo *tbmcir.TbImageInfo) (*tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
...
func RegVirtualMachineImage(nameSpaceID string, registType string, virtualMachineImageRegInfo *tbmcir.TbImageReq) (*tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
	fmt.Println("RegVirtualMachineImage ************ : ")
	log.Println("*********************** virtualMachineImageRegInfo :")
	log.Println(virtualMachineImageRegInfo)
	log.Println("***********************")
	if registType == "" {
...
// VirtualMachineImage 등록 registeringMethod = imageID
// 생성시 action에 regist
func RegVirtualMachineImageWithInfo(nameSpaceID string, registType string, virtualMachineImageRegInfo *tbmcir.TbImageInfo) (*tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
	fmt.Println("RegVirtualMachineImage ************ : ")
	if registType == "" {
		registType = "registerWithId" // registerWithId 또는 registerWithInfo
	}
...
func DelAllVirtualMachineImage(nameSpaceID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
...
func DelVirtualMachineImage(nameSpaceID string, virtualMachineImageID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	// if ValidateString(VirtualMachineImageID) != nil {
	if len(virtualMachineImageID) == 0 {
		log.Println("ImageID 가 없으면 해당 namespace의 모든 image가 삭제되므로 처리할 수 없습니다.")
		return webStatus, frameworkmodel.WebStatus{StatusCode: 4040, Message: "ImageID 가 없으면 해당 namespace의 모든 image가 삭제되므로 처리할 수 없습니다."}
	}
...
func GetInspectResourceList(inspectResource *tbcommon.RestInspectResourcesRequest) (*tbmcis.InspectResource, frameworkmodel.WebStatus) {
	...
	if err != nil {
		...
		//return &inspectResourcesResponse, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
		return &inspectResourcesResponse, frameworkmodel.WebStatus{StatusCode: 500, Message: failResultInfo.Message}
	}

	json.NewDecoder(respBody).Decode(&inspectResourcesResponse)
	fmt.Println(inspectResourcesResponse)

	return &inspectResourcesResponse, frameworkmodel.WebStatus{StatusCode: respStatus}
...
*/
func GetInspectResourcesOverview() (*tbmcis.InspectResourceAllResult, frameworkmodel.WebStatus) {
	...
	if err != nil {
		...
		log.Println(failResultInfo)
		//return &inspectResourcesResponse, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
		return &inspectResourceAllResult, frameworkmodel.WebStatus{StatusCode: 500, Message: failResultInfo.Message}
	}

	json.NewDecoder(respBody).Decode(&inspectResourceAllResult)
	fmt.Println(inspectResourceAllResult)

	return &inspectResourceAllResult, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func RegCspResources(resourcesRequest *tbcommon.RestRegisterCspNativeResourcesRequest, optionParam string) (*tbmcis.RegisterResourceResult, frameworkmodel.WebStatus) {
	...
	if err != nil {
		...
		log.Println(failResultInfo)
		//return &inspectResourcesResponse, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
		return &registerResourceResult, frameworkmodel.WebStatus{StatusCode: 500, Message: failResultInfo.Message}
	}

	json.NewDecoder(respBody).Decode(registerResourceResult)
	fmt.Println(registerResourceResult)

	return &registerResourceResult, frameworkmodel.WebStatus{StatusCode: respStatus}

}

func RegCspResourcesAll(resourcesRequest *tbcommon.RestRegisterCspNativeResourcesRequestAll, optionParam string) (*tbmcis.RegisterResourceAllResult, frameworkmodel.WebStatus) {
...
func GetLoadCommonResource() (tbcommon.TbSimpleMsg, frameworkmodel.WebStatus) {
...
// VM Image 조회
func LookupVirtualMachineImageList(connectionName string) (tbmcir.SpiderImageInfos, frameworkmodel.WebStatus) {
	...
	return lookupImageList["image"], frameworkmodel.WebStatus{StatusCode: respStatus}
}

// deprecated :  imageId 받는 것에서 connection 받는 것 까지로 변경 됨
// func LookupVirtualMachineImageData(virtualMachineImageID string) (*tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
...
func LookupVirtualMachineImageData(restLookupImageRequest *tbmcir.RestLookupImageRequest) (*tbmcir.SpiderImageInfo, frameworkmodel.WebStatus) {
...
// csp에 등록된 정보조회.
func FetchVirtualMachineImageList(nameSpaceID string) ([]tbcommon.TbSimpleMsg, frameworkmodel.WebStatus) {
	...
	// return fetchImageList["image"], frameworkmodel.WebStatus{StatusCode: respStatus}
	return nil, frameworkmodel.WebStatus{StatusCode: respStatus} // TODO : simpleMsg return 하도록 변경할 것
}

// VirtualMachineImage 상세 조회
func SearchVirtualMachineImageList(nameSpaceID string, restSearchImageRequest *tbmcir.RestSearchImageRequest) ([]tbmcir.TbImageInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return virtualMachineImageInfo.Image, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	return virtualMachineImageInfo.Image, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// VMSpec 목록 조회
func GetVmSpecInfoList(nameSpaceID string) ([]tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	...
	return vmSpecList.Spec, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func GetVmSpecInfoListByID(nameSpaceID string, filterKeyParam string, filterValParam string) ([]string, frameworkmodel.WebStatus) {
	fmt.Println("GetVMSpecInfoList ************ : ")
	var originalUrl = "/ns/{nsId}/resources/spec"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
	...
	return vmSpecList.IDList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func GetVmSpecInfoListByOption(nameSpaceID string, optionParam string, filterKeyParam string, filterValParam string) ([]tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	log.Println("GetVMSpecInfoList ************ : ")
	var originalUrl = "/ns/{nsId}/resources/spec"
	var paramMapper = make(map[string]string)
	paramMapper["{nsId}"] = nameSpaceID
...
// VMSpec 상세 조회
func GetVmSpecInfoData(nameSpaceID string, vmSpecID string) (*tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &vmSpecInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	...
	return &vmSpecInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// VMSpecInfo 등록
func RegVmSpec(nameSpaceID string, specregisteringMethod string, vmSpecRegInfo *tbmcir.TbSpecReq) (*tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	fmt.Println("RegVMSpec ************ : ")
	log.Println("**********namespace ID : ", nameSpaceID)
	log.Println("**************************************")
...
func RegVmSpecWithInfo(nameSpaceID string, specregisteringMethod string, vmSpecRegInfo *tbmcir.TbSpecInfo) (*tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	fmt.Println("RegVMSpec ************ : ")
	if specregisteringMethod == "" {
		specregisteringMethod = "registerWithInfo" // registerWithInfo or Else 이므로 registerWithInfo 를 넣거나 아니거나.
	}
...
func UpdateVMSpec(nameSpaceID string, vmSpecRegInfo *tbmcir.TbSpecInfo) (*tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
...
func DelAllVMSpec(nameSpaceID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
...
// VMSpec 삭제
func DelVMSpec(nameSpaceID string, vmSpecID string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	// if ValidateString(VMSpecID) != nil {
	if len(vmSpecID) == 0 {
		log.Println("specID 가 없으면 해당 namespace의 모든 image가 삭제되므로 처리할 수 없습니다.")
		return webStatus, frameworkmodel.WebStatus{StatusCode: 4040, Message: "specID 가 없으면 해당 namespace의 모든 image가 삭제되므로 처리할 수 없습니다."}
	}
...
func LookupVmSpecInfoList(connectionName *tbcommon.TbConnectionName) (tbmcir.SpiderSpecInfos, frameworkmodel.WebStatus) {
	...
	return vmSpecList["vmspec"], frameworkmodel.WebStatus{StatusCode: respStatus}

}

// deprecated 호출경로 변경
// func LookupVmSpecInfoData(vmSpecName string) (*tbmcir.RestGetAllSpecResponse, frameworkmodel.WebStatus) {
...
func LookupVmSpecInfoData(restLookupSpecRequest *tbmcir.RestLookupSpecRequest) (*tbmcir.SpiderSpecInfo, frameworkmodel.WebStatus) {
...
// Fetch는 결과만 return
func FetchVmSpecInfoList(nameSpaceID string) (*tbcommon.TbSimpleMsg, frameworkmodel.WebStatus) {
...
// 오래걸리므로 비동기로 처리
func FetchVmSpecInfoListByAsync(nameSpaceID string, c buffalo.Context) {
...
// spec들을 filterling
func FilterVmSpecInfoList(nameSpaceID string, vmSpecRegInfo *tbmcir.TbSpecInfo) ([]tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	...
	return filterSpecList.SpaceInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

// resourcesGroup.POST("/vmspec/filterspecsbyrange", controller.FilterVmSpecListByRange)
// namespace의 spec 조회
// 기본값은 system-purpose-common-ns 이며 spec load를 해야 사용가능
// 특정 namespace를 주면 해당 ns에 있는 spec을 filter
func FilterVmSpecInfoListByRange(nameSpaceID string, vmSpecRangeMinMax *tbmcir.FilterSpecsByRangeRequest) ([]tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	webStatus := frameworkmodel.WebStatus{}
	...
	if err != nil {
		fmt.Println(err)
		return resultInfo.SpaceInfo, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	// defer body.Close()
	respBody := resp.Body
	respStatus := resp.StatusCode
	...
	if respStatus != 200 && respStatus != 201 {
		...
		return resultInfo.SpaceInfo, frameworkmodel.WebStatus{StatusCode: respStatus, Message: failResultInfo.Message}
	}
	...
	return resultInfo.SpaceInfo, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func DelDefaultResources(nameSpaceID string) (*tbcommon.TbIdList, frameworkmodel.WebStatus) {
	...
	if err != nil {
		fmt.Println(err)
		return &idList, frameworkmodel.WebStatus{StatusCode: 500, Message: err.Error()}
	}
	// return body, err
	respBody := resp.Body
	respStatus := resp.StatusCode
	...
	if respStatus != 200 && respStatus != 201 {
		...
		json.NewDecoder(respBody).Decode(&failResultInfo)
		log.Println("ResultMessage : " + failResultInfo.Message)
		return &idList, frameworkmodel.WebStatus{StatusCode: respStatus, Message: failResultInfo.Message}
	}

	json.NewDecoder(respBody).Decode(&idList)
	fmt.Println(idList)

	return &idList, frameworkmodel.WebStatus{StatusCode: respStatus}
}

func LoadDefaultResources(nameSpaceID string, optionParam string, connectionName string) (frameworkmodel.WebStatus, frameworkmodel.WebStatus) {
...
// 실제로는 추천 vmspec 목록
func GetRecommendVmSpecList(mcisDeploymentPlan *tbmcis.DeploymentPlan) ([]tbmcir.TbSpecInfo, frameworkmodel.WebStatus) {
	...
	if respStatus != 200 && respStatus != 201 { // 호출은 정상이나, 가져온 결과값이 200, 201아닌 경우 message에 담겨있는 것을 WebStatus에 set
		...
		return returnVmSpecs, frameworkmodel.WebStatus{StatusCode: respStatus, Message: failResultInfo.Message}
	}
