...
/** ready    : 구현 완료, 실행 가능
 *  wip      : 작업 중 (구현 진행 중)
 *  bypass   : 미구현·미지원 기능 — 시나리오에서 skip 후 다음 스텝 진행
 *  todo     : 구현 예정 (spec 파일 없음)
 *  deprecated: 더 이상 사용하지 않음
 */
export type TCStatus = 'ready' | 'wip' | 'bypass' | 'todo' | 'deprecated';
...
// ── 시나리오 상태 ────────────────────────────────────────────────────────────
/** ready   : 전체 스텝 실행 가능
 *  partial : 일부 스텝만 ready (나머지 bypass/todo)
 *  wip     : 작업 중
 *  todo    : 구현 예정
 *  deprecated: 더 이상 사용하지 않음
 */
