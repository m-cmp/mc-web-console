 ...
 *
 * ── bypass 안전 처리 ──────────────────────────────────────────────
 *   TC-INFRA-MCI-03 가 bypass 됐을 때 → getOrDefault 로 fallback MCI 사용
 */
...
test.describe('TC-WORKLOAD-MCI-01: MCI 터미널 접속·명령 실행', () => {
  ...
  test.beforeAll(() => {
    if (store) {
      if (store.wasBypassed('TC-INFRA-MCI-03')) {
        // TC-INFRA-MCI-03 이 bypass → fallback MCI 사용
        mciId   = store.getOrDefault<string>('mciId',   p.mciId   as string ?? '');
        mciName = store.getOrDefault<string>('mciName', p.mciName as string ?? 'fallback-mci');
        console.warn(`[TC-WORKLOAD-MCI-01] TC-INFRA-MCI-03 bypass → fallback mciId='${mciId}', mciName='${mciName}'`);
      } else {
  ...
  test('UI: MCI 워크로드 화면 → 터미널 → 명령 실행', async ({ page }) => {
    ...
    // mciId 없으면 단독 실행 불가 — 경고 후 skip
    if (!mciId && !store) {
      console.warn(`[${tag}] mciId 없음 — 단독 실행 skip (PW_mciId 주입 필요)`);
      test.skip();
      return;
    }
